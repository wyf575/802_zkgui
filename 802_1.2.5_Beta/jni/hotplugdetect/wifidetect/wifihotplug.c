...
static void *WlanWorkProc(void *pdata)
{
    ...
    if (isWlanInsmode)
    {
		...
		g_connThreadRun = 1;
		pthread_create(&g_connThread, NULL, WifiConnectProc, NULL);//5min排除
		if (!g_connThread)
		{
		...
		g_scanThreadRun = 1;
		pthread_create(&g_scanThread, NULL, WifiScanProc, NULL);//5min排除
		if (!g_scanThread)
		{
			printf("create check hotplug thread failed\n");
			return NULL;
...
int Wifi_StartCheckHotplug()
{
	...
	g_wlanThreadRun = 1;
	pthread_create(&g_wlanThread, NULL, WlanWorkProc, NULL);//5min排除
	if (!g_wlanThread)
	{
		printf("create check hotplug thread failed\n");
		return -1;
