...
http_trans_read_into_buf(http_trans_conn *a_conn)
{
    ...
    /* read in some data */
    if ((a_conn->last_read = l_read = read(a_conn->sock, &a_conn->io_buf[a_conn->io_buf_alloc],l_bytes_to_read)) < 0)
    {
        if (errno == EINTR)
            l_read = 0;
        else
...
http_trans_write_buf(http_trans_conn *a_conn)
{
    ...
    /* write out some data */
    printf("a_conn:%s\n",a_conn->io_buf);
    if ((a_conn->last_read = l_written = write (a_conn->sock, &a_conn->io_buf[a_conn->io_buf_io_done],a_conn->io_buf_io_left)) <= 0)
    {
        if (errno == EINTR)
            l_written = 0;
        else
