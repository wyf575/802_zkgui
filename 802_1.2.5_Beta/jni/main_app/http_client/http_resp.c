    ...
    while (l_done == 0)
    {
        ...
        else if(l_state == reading_value)
        {
            ...
            if ((*l_cur_ptr == '\r') || (*l_cur_ptr == '\n'))
            {
                ...
                if (l_header_len == 0)
                {
                    http_hdr_set_value_no_nts(a_resp->headers,l_last_header,l_last_header_len,l_start_value,l_value_len);
                }
                else
                {
                    http_hdr_set_value_no_nts(a_resp->headers,l_start_header,l_header_len,l_start_value,l_value_len);
...
int
http_resp_read_body(http_resp *a_resp,http_req *a_req,http_trans_conn *a_conn)
{
    int      l_return = 0;
    ...
    l_content_length =
    http_hdr_get_value(a_resp->headers, http_hdr_Content_Length);
    /* check to see if there's a transfer encoding */
    l_transfer_encoding = http_hdr_get_value(a_resp->headers, http_hdr_Transfer_Encoding);
    /* check to see if the connection header is set */
    l_connection = http_hdr_get_value(a_resp->headers, http_hdr_Connection);
...
read_chunk(http_trans_conn *a_conn)
{
    ...
    if (a_conn->chunk_len == 0)
    {
        ...
        buffer */
        do {
            if ((l_end_chunk_hdr = http_trans_buf_has_patt(a_conn->io_buf, a_conn->io_buf_alloc,"\r\n", 2)) == NULL)
            {
                l_rv = http_trans_read_into_buf(a_conn);
                if (l_rv == HTTP_TRANS_ERR)
                    return HTTP_TRANS_ERR;
...
static int
read_body_chunked(http_resp *a_resp,http_req *a_req,http_trans_conn *a_conn)
{
    int   l_rv = 0;
    int   l_done = 0;
    ...
    do
    {
        ...
        if (a_conn->chunk_len > 0)
        {
            ...
            {
                a_resp->body = realloc(a_resp->body,(a_resp->body_len + a_conn->chunk_len));
...
void
http_resp_flush(http_resp *a_resp,http_trans_conn *a_conn)
{
    flush_response_body(a_resp, a_conn);
}

static int
read_body_content_length(http_resp *a_resp,http_req *a_req,http_trans_conn *a_conn)
{
    int l_len = 0;
    int l_left_to_read = 0;
    int l_rv = 0;
...
static int
read_body_standard(http_resp *a_resp,http_req *a_req,http_trans_conn *a_conn)
{
    int l_rv = 0;
